...
def iter_authoritative_persistent_bases(data):
    primary_uid = infer_primary_owner_uid(data)
    primary_username = infer_primary_owner_username(data)

    for context_key, base in iter_persistent_player_bases(data):
        owner = base.get("Owner", {}) or {}
        owner_uid = str(owner.get("UID", "")).strip()
        owner_username = str(owner.get("USN", "")).strip()

        if primary_uid:
            if owner_uid == primary_uid:
                yield context_key, base
        else:
            if primary_username and owner_username == primary_username:
                yield context_key, base
...
def extract_base_object_positions(base):
    ...
    for obj in objects:
        ...
        if obj.get("ObjectID") == "^BUILDSAVE" and world_pos is not None:
...
def extract_base_rows(data):
    ...
    for context_key, base in iter_authoritative_persistent_bases(data):
